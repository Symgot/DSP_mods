...
## **KRITISCHE DOKUMENTATIONS-REGEL**

**Diese Regel hat höchste Priorität und überschreibt alle anderen Anweisungen bezüglich Dokumentationserstellung:**

> **Alle Dokumentation und Diskussion erfolgt im PR, NICHT im Issue.**

**Konkret bedeutet das:**

1. **KEINE ERSTELLUNG** folgender Inhalte ohne EXPLIZITE Erwähnung im Issue:
   - README.md, INSTALL.md, USAGE.md, CONTRIBUTING.md
   - Handlungsanweisungen, Installationsanweisungen
   - Zusammenfassungen (Summaries), Fixes-Beschreibungen
   - Changelog-Einträge außerhalb von PR-Beschreibungen
   - Dokumentations-Dateien jeglichen Typs und Formats

2. **PR-ABSCHLUSSBERICHT** (anstatt Zusammenfassung):
   - Tabellarische Auflistung ALLER Änderungen
   - Spalten: Datei | Änderungstyp | Detaillierte Änderung | Commit-SHA | Quelle-URL
   - So detailliert wie möglich (Zeilennummern, Funktionsnamen, etc.)

3. **COMMIT-MESSAGES**:
   - Jeder Commit MUSS Quellen-URL(s) enthalten
   - Format: `<Typ>: <Beschreibung> (fixes #<issue>) - Source: <URL>`
   - Ermöglicht Verifikation und Nachvollziehbarkeit

4. **DATEI-REFERENZIERUNG**:
   - In erstellten/bearbeiteten Dateien MUSS zu jeder Änderung der Commit referenziert werden
   - Format als Kommentar: `# Modified in commit <sha>: <Beschreibung> - Source: <URL>`

5. **ISSUE-KOMMENTARE**:
   - Nur minimale Benachrichtigung: "Lösung in PR #<nummer> - Alle Details und Dokumentation dort."
   - KEINE technischen Details, Änderungslisten oder Erklärungen im Issue

**Diese Regeln sind nicht verhandelbar und gelten für JEDEN Issue- und PR-Workflow.**
...
- Generiert vollständige Lösung gemäß Policy 1 und 2:
  ...
  - **Für Documentation**: Vollständig aktualisierte Dokumentation nur wenn AUSDRÜCKLICH im Issue gefordert
  - **Für Enhancements**: Vollständige Code-Verbesserungen
- **KRITISCH**: Keine eigenständige Erstellung von Dokumentationen (README, Installation Guides, Summaries, etc.) außer wenn EXPLIZIT im Issue gefordert
...
- **WICHTIG**: Jede Änderung muss mit Quellen-URL versehen werden, um Verifikation und Nachvollziehbarkeit zu ermöglichen
...
- **KRITISCH**: Jeder Commit MUSS die Quellen-URL(s) im Commit-Message enthalten zur Verifikation und Nachvollziehbarkeit.
- Format: "Fix: <Beschreibung> (fixes #<issue>) - Source: <URL>"
- Bei mehreren Dateien: Separate Commits pro logischer Änderung mit jeweiliger Quelle.

#### Funktion `create_solution_pr(issue_number, branch_name, solution_data)`
- Erstellt PR mit vollständiger Lösung.
- **WICHTIG**: Alle Dokumentation und Diskussion erfolgt im PR, NICHT im Issue.
- PR-Body enthält:
  - Bezug zum Issue
  - **TABELLARISCHE AUFLISTUNG** aller Änderungen (KEINE Zusammenfassung):
    - Tabelle mit Spalten: Datei | Änderung | Commit-SHA | Quelle
    - So detailliert wie möglich
  - Quellen / References Sektion mit stabilen blob URLs für jede Änderung
  - Erklärung zur Verifikation
- **KEINE** Zusammenfassung, Summaries oder allgemeine Beschreibungen
- Jede Zeile der Tabelle referenziert den zugehörigen Commit mit Quellen-URL

#### Funktion `post_issue_comment(issue_number, body)`
- Kommentiert Issue MINIMAL mit PR-Link.
- **WICHTIG**: KEINE Lösungsvorschläge, Zusammenfassungen oder Dokumentation im Issue.
- Alle Dokumentation und Diskussion erfolgt AUSSCHLIEßLICH im PR.
- Kommentar enthält nur: "Lösung in PR #<nummer> - Alle Details und Dokumentation dort."
...
- Vollständiger Issue-Bearbeitungs-Workflow:
  ...
  4. Generiere vollständige Lösung (Policy 1: vollständig, Policy 2: keine Planung)
     - **KEINE** eigenständige Erstellung von Dokumentationen außer wenn EXPLIZIT gefordert
  5. Erstelle Solution-Branch mit Commits die Quellen enthalten
  6. Erstelle PR mit allen Änderungen (Policy 7: vollständige Dateien)
     - PR enthält tabellarische Auflistung aller Änderungen + Commits + Quellen
     - In bearbeiteten Dateien werden Commit-Referenzen hinterlegt
  7. Kommentiere Issue MINIMAL mit PR-Link (KEINE Dokumentation im Issue)
  8. Bei Erfolg: Schließe Issue oder markiere als "resolved"

**KRITISCHE REGEL**: Alle Dokumentation und Diskussion erfolgt im PR, NICHT im Issue.


### Commit- und Datei-Referenzierungs-Richtlinien

**Zwingend erforderliche Praktiken:**

#### Commit-Messages
- **Format**: `<Typ>: <Kurzbeschreibung> (fixes #<issue>) - Source: <URL>`
- **Beispiel**: `Fix: Validate version tags correctly (fixes #123) - Source: https://github.com/org/repo/blob/abc123/docs/api.md`
- Jeder Commit MUSS eine verifizierbare Quelle enthalten
- Bei mehreren Quellen: Alle URLs im Commit-Body auflisten

#### Datei-Referenzierung
- In jeder bearbeiteten/erstellten Datei MUSS zur jeder Änderung der zugehörige Commit referenziert werden
- **Format als Kommentar**: `# Modified in commit <sha>: <Beschreibung> - Source: <URL>`
- **Beispiel**:
  ```python
  # Modified in commit abc123def: Fixed version tag validation
  # Source: https://github.com/org/repo/blob/main/docs/api.md
  def validate_reference(url: str) -> bool:
      VERSION_REGEX = re.compile(r"\bv?\d+\.\d+(\.\d+)?\b")  # Added in abc123def
      ...
  ```

#### PR-Abschlussbericht
- **KEINE Zusammenfassung**, stattdessen:
- Tabellarische Auflistung mit folgenden Spalten:
  - **Datei**: Vollständiger Pfad
  - **Änderungstyp**: Added/Modified/Deleted
  - **Änderung**: Detaillierte Beschreibung (Zeilen, Funktionen, etc.)
  - **Commit-SHA**: Kurz-SHA (7 Zeichen)
  - **Quelle**: URL zur Verifikation
- Beispiel-Tabelle:
  ```markdown
  | Datei | Änderungstyp | Änderung | Commit | Quelle |
  |-------|--------------|----------|--------|--------|
  | src/validator.py | Modified | Zeile 5-12: Version-Tag-Regex hinzugefügt | abc123d | [API Docs](https://...) |
  | tests/test_validator.py | Modified | Zeile 15-18: Testfälle für v-Tags | abc123d | [Test Guide](https://...) |
  ```

#### Dokumentations-Verbot
- **KEINE** eigenständige Erstellung folgender Dateien ohne EXPLIZITE Anforderung im Issue:
  - README.md, INSTALL.md, USAGE.md
  - Changelog-Einträge außerhalb von PR-Beschreibungen
  - Zusammenfassungen, Summaries, Guides
  - Handlungsanweisungen, Installationsanleitungen
- **Alle Dokumentation erfolgt AUSSCHLIEßLICH im PR**, nicht im Issue
...
```python
post_issue_comment(issue_number, f"Lösung in PR #{pr_number} - Alle Details und Dokumentation dort.")
# KEINE Zusammenfassung, KEINE Dokumentation im Issue
# Alle Dokumentation erfolgt AUSSCHLIEßLICH im PR
```
...
- [ ] **KRITISCHE DOKUMENTATIONS-REGEL**: Alle Dokumentation im PR, nicht im Issue?
  - Keine eigenständige Erstellung von README, Summaries, Installationsanleitungen?
  - PR-Abschlussbericht ist tabellarisch (nicht Zusammenfassung)?
  - Issue-Kommentar ist minimal (nur PR-Link)?
  - Alle Commits enthalten Quellen-URLs?
  - Bearbeitete Dateien referenzieren Commits mit Quellen?
...
- Commit: "Fix: Validate version tags correctly (fixes #123) - Source: https://github.com/org/repo/blob/main/docs/validation-spec.md"
- PR-Body:
```markdown
## Bezug
Fixes #123
...
| Datei | Änderungstyp | Änderung | Commit | Quelle |
|-------|--------------|----------|--------|--------|
| src/validator.py | Modified | Zeile 5-12: Version-Tag-Regex `VERSION_REGEX` hinzugefügt für Erkennung von v1.2.3 Patterns | abc123d | [Validation Spec](https://github.com/org/repo/blob/main/docs/validation-spec.md) |
| tests/test_validator.py | Modified | Zeile 15-18: Zwei neue Testfälle `test_version_tag_validation()` für Version-Tag-URLs | abc123d | [Testing Guide](https://github.com/org/repo/blob/main/docs/testing.md) |
...
```markdown
Lösung in PR #456 - Alle Details und Dokumentation dort.
```

**WICHTIG:** Alle technischen Details, Änderungen, Quellen und Diskussion erfolgen AUSSCHLIEßLICH im PR.
